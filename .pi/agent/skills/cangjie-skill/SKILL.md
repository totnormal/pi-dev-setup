...
当用户不只要"做成 skill 库",而是要"读了这些内容**帮我看看怎么用在我的公司/项目上 / 据此给我落地计划**"时,
在主流水线(阶段 5)之后追加一层针对当前用户处境的批判性适配。详见 `references/context-adaptation.md`
(找默认起点 → 逐条原则分类 free-win/holds/N-A/dangerous → 补缺失层并标清署名 → 按用户 #1 痛点重排序;
产出 foundation/analysis/roadmap 三份职责分离的文档;架构选择显式挂成"待用户决策")。
