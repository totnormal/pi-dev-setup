...
DEFAULT_KB_ROOT = os.environ.get("MINIMBA_KB_ROOT", MACOS_DEFAULT_KB_ROOT)
...
def _rtf_to_text(path: Path) -> str:
    ...
    raise RuntimeError(
        "RTF extraction requires macOS /usr/bin/textutil or Linux `unrtf`.\n"
        "On Ubuntu: apt-get install -y unrtf\n"
        "Or re-run without --include-rtf."
    )
...
def main() -> int:
    ...
    if not kb_root.exists():
        print(f"[kb_materialize] KB root not found: {kb_root}", file=sys.stderr)
        print("[kb_materialize] Set MINIMBA_KB_ROOT or pass --kb-root.", file=sys.stderr)
        return 2
