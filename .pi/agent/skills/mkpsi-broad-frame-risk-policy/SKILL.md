---
name: mkpsi-broad-frame-risk-policy
description: |
  Use when: Repeated independent survivable risks are evaluated one by one and loss aversion blocks po
  sitive expected value. Trigger phrases: one-off risk,, extended warranty,, deductible,, portfolio,, 
  each division,, positive expected value,. NOT for: Do not apply to ruinous losses, correlated bets, 
  or long shots with tiny win probability. Respect user risk constraints.
source_book: |
...
source_chapter: |
  Ch. 31 “Risk Policies,” “Broad or Narrow?” and “Risk Policies”
tags: [mkpsi-v2, bias, p2]
related_skills: [mkpsi-orchestrator, mkpsi-output-verifier, mkpsi-manipulation-ethics-gate, mkpsi-reframe-equivalent-options, mkpsi-test-learn-adapt-rct, mkpsi-reference-class-forecast]
disable-model-invocation: true
---

# Aggregate repeated risks with a precommitted risk policy.

## R — Reading

> “A risk policy is a broad frame that embeds a particular risky choice in a set of similar choices.”
>
> — Source: Ch. 31 “Risk Policies,” “Broad or Narrow?” and “Risk Policies”

**Source mechanism:** For recurring small-to-moderate risks, define a policy in advance (accept positive expected value if independent and survivable; avoid low-value warranties/insurance), then apply it consistently instead of deciding case by case.
...
For recurring small-to-moderate risks, define a policy in advance (accept positive expected value if independent and survivable; avoid low-value warranties/insurance), then apply it consistently instead of deciding case by case.
...
### Case 1: Samuelson’s friend rejected one lose-$100/win-$200 gamble but wanted 100 such be
- **Problem:** Samuelson’s friend rejected one lose-$100/win-$200 gamble but wanted 100 such bets; aggregation makes risk attractive.
...
### Case 2: Division managers rejected individual risky bets while the CEO wanted all 25 to 
- **Problem:** Division managers rejected individual risky bets while the CEO wanted all 25 to accept because portfolio aggregation reduced total risk.
...
1. Repeated independent survivable risks are evaluated one by one and loss aversion blocks positive expected value.
...
- Sibling confusion risks: Not the same as reckless risk-taking; requires independence, survivability, and repeated class of bets.

---

## E — Execution steps

1. **Apply the source method: For recurring small-to-moderate risks, define a policy in advance (accept positive expected value if independent and survivable; avoid low-value warranties/insurance), then apply it consistently instead of deciding case by case.**
...
- **Selected mechanism:** `mkpsi-broad-frame-risk-policy` — Aggregate repeated risks with a precommitted risk policy.
...
- Do not apply to ruinous losses, correlated bets, or long shots with tiny win probability. Respect user risk constraints.
...
Not the same as reckless risk-taking; requires independence, survivability, and repeated class of bets.
...
- composes-with: /mkpsi-orchestrator, /mkpsi-output-verifier, /mkpsi-manipulation-ethics-gate, /mkpsi-reframe-equivalent-options, /mkpsi-test-learn-adapt-rct, /mkpsi-reference-class-forecast
