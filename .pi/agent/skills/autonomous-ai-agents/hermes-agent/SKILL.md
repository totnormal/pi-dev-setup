...
| `vision` | `provider` (openrouter, anthropic, google, openai, etc.), `model` (e.g., google/gemma-3-4b-it:free) |
...
| Platform setup | `hermes gateway setup` or [Messaging docs](https://hermes-agent.nousresearch.com/docs/user-guide/messaging-agent.nousresearch.com/docs/user-guide/messaging/) |
...
| Video analysis guide | `skill_view(name="hermes-agent", file_path="references/video-analysis.md")` |
...
## Troubleshooting Tooling Inconsistencies

When documentation references a toolset that doesn't exist, use the investigation guide:
`skill_view(name="hermes-agent", file_path="references/investigating-missing-toolsets.md")`

This covers checking toolsets.py, tools_config.py, exclusion lists, and resolving the mismatch.
